...
const emptyForm = { name: '', address: '', latitude: '', longitude: '', totalPorts: 2, powerKw: 150, tariffPerKwh: 0.35 };
...
export default function StationAdmin() {
  ...
  const handleCommission = async (stationId) => {
    try {
      await adminAPI.commissionStation(stationId);
      loadStations();
    } catch (err) {
      alert(err.response?.data?.message || 'Failed to commission station');
    }
  };
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input name="address" value={form.address} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Latitude</label>
              <input name="latitude" type="number" step="any" value={form.latitude} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Longitude</label>
              <input name="longitude" type="number" step="any" value={form.longitude} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required />
            </div>
            <div>
