...
export default function StationDetail() {
  ...
  const handleStartCharging = async (portId) => {
    setStarting(true);
    setError('');
    try {
      await sessionsAPI.start({ stationId: id, portId, batteryCapacityKwh: battery });
      navigate('/charging');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to start charging');
    } finally {
      setStarting(false);
    }
  };
